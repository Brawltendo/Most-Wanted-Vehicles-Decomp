...
{
	return speed * chassis.data->AERO_COEFFICIENT * 2000.f;
}
...
float Physics::Info::EngineInertia(const Attrib::Gen::engine& engine, const bool loaded)
{
	...
	if (loaded)
		...
		inertia_mul = 0.35f;
	return inertia_mul * (engine.data->FLYWHEEL_MASS * 0.025f + 0.25f);
}

// MATCHING
Physics::Info::eInductionType Physics::Info::InductionType(const Attrib::Gen::induction& induction)
{
	if (induction.data->HIGH_BOOST > 0.f || induction.data->LOW_BOOST > 0.f)
	{
